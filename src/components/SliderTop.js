...
function SamplePrevArrow({ onClick }) {
  return (
    <button
                  ...
                  className="prev hidden sm:block absolute -left-16 top-1/2 transform -translate-y-1/2 group lg:p-4 p-3 rounded-full bg-primary-dark hover:bg-secondary-dark focus:outline-none transition-colors duration-500"
                >
...
function SampleNextArrow({ onClick }) {
  return (
    <button
                  type="button" onClick={onClick}
                  className="next hidden sm:block absolute -right-16 top-1/2 transform -translate-y-1/2 group lg:p-4 p-3 rounded-full bg-primary-dark hover:bg-secondary-dark focus:outline-none transition-colors duration-500"
                > 
                  <span className="animate-ping absolute left-0 top-0 inline-flex h-full w-full rounded-full bg-primary-dark opacity-50"></span>
                  <svg
...
const SliderTop = ({topics, isReverse}) => {
  ...
  let settings = {
    ...
    customPaging: (i)  => {
      return (
      <div className={`${i === pages.currentSlide ? 'bg-primary-dark' : 'bg-primary-light'} h-3 w-3 mx-2 my-4 border border-primary-dark rounded-full hover:bg-primary-dark`} 
      ></div>
    )}
  };
